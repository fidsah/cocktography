...
#our @EXPORT_OK = qw(enchode_string dechode_string);
...
sub new {
		...
		# Load kontol chodes
		open(my $kontolgrip, "<", $kontolfilename) or croak "Couldn't open kontol chode file FUCK $kontolfilename!";
		while(<$kontolgrip>) {
			chomp;
...
sub cyphallicize ($$$;$) {
	...
	if ($mode == $CYPHALLIC_METHOD{"THIN"}) {
		foreach my $character (split("", $input)) {
			#print "thinchar $character|" . (ord $character) . " -=- " . $thinchodes[ord $character] . "\n";
		...
		foreach my $character (split("", $input)) {
			#print "widechar $character|" . (ord $character) . "\n";
...
sub decyphallicize($$) {
	...
	foreach my $chode (@chodes) {
		...
		elsif(exists $widechodelist{$chode}) { $raw .= chr($widechodelist{$chode}); }
